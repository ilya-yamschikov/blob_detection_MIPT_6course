...
\section{Задача:}Изначально задача ставится как выделение ярких пятен на изображении, каждый пиксель которого является комплексным числом и несет значения амплитуды и фазы принятого сигнала. В данной работе яркие пятна ищутся на основе только амплитудной части.

Соответственно требуется на монохромном изображении выделить светлые пятна - однородные яркие участки с резким изменением градиента яркости на границе. Ключевые слова для подобных алгоритмов - \textit{blob detection}.

Имеется изображение $\{Z, \Phi\}$ - действительная и мнимая части. Оно преобразуется к $P: p_{i,j} = \sqrt{z_{i,j}^2 + \phi_{i,j}^2}$ и строится алгоритм для поиска ярких пятен на изображении $P = \{p_{ij}\}, i=1,\dots,\xs, j=1,\dots,\ys$ где $p_{ij} \in B$ - яркость пиксела. Здесь $B \in \mathbb{R}_+$ - возможные значения яркости.

\section{Алгоритм:}
\subsection{Описание:}
Алгоритм должен получать на вход изображение $P$ и, возможно, набор параметров. На выходе требуется получить бинарную матрицу $R$ в которой единицам соответствуют центры найденных пятен.
...
\textbf{Обозначения:}
\begin{itemize}
    \item $P$ - входное изображение
    \item $R$ - матрица, совпадающая по размеру с $P$, с элементами $r_{ij} \in \{0,1\}$, в которой единицы соответствуют центрам ярких пятен.
